...
func _ready() -> void:
	emergencies.append(Emergency.new( Vector2(720, 1060) ))
	...
	emergencies.append(Emergency.new( Vector2(480, 1400) ))
	emergencies[1].add_dialogue_option(DialogueOption.new( "Gdzie pan jest?", "", "Biuro Polyservers, Zielone Tulipany 82 ", "", "Adres: Zielone Tulipany 82"))
	emergencies[1].add_dialogue_option(DialogueOption.new( "Ile osób jest poszkodowanych?", "", "Chyba nikt, ale jedna starsza osoba utknęła w budynku", "", "Ilość poszkodowanych: 0"))
	emergencies[1].add_dialogue_option(DialogueOption.new( "Co się stało?", "", "Serwerownia nagle zaczęła się palić, nie wygląda to na zwykły pożar", "", "Przyczyna: Możliwe podpalenie serwerowni"))
...
func create_buttons(emergency : Emergency):
	get_parent().update_transcript("Dzień dobry, straż pożarna.")
...
func option_chosen(option : DialogueOption, coordinates = Vector2.ZERO):
	...
	if coordinates != Vector2.ZERO:
		get_tree().current_scene.map_container.spawn_event('Fire', coordinates)
	if not enable_buttons():
		get_tree().current_scene.side_panel.enable_buttons()
