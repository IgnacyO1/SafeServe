...
# Słownik do śledzenia w którym chunku ostatnio był dany gracz (ID_instancji -> Vector2i)
var player_last_chunks = {}
...
func initialize_map(start_pos: Vector2):
	...
	chunk_roads.clear()
	major_nodes.clear()
	player_last_chunks.clear() # Czyścimy pamięć podręczną pozycji graczy
	
	update_chunks()

func _process(_delta):
	# Pobieramy wszystkich graczy w grupie
	var players = get_tree().get_nodes_in_group("police")
	if players.is_empty() and player:
		players = [player]
		
	var trigger_update = false
	var current_valid_ids = []
	
	# Sprawdzamy, czy KTÓRYKOLWIEK gracz przeszedł do innego chunku
	for p in players:
		if not is_instance_valid(p): continue
		var rid = p.get_instance_id()
		current_valid_ids.append(rid)
		
		var p_chunk = Vector2i(
			int(floor(p.global_position.x / chunk_size_px)),
			int(floor(p.global_position.y / chunk_size_px))
		)
		
		if not player_last_chunks.has(rid) or player_last_chunks[rid] != p_chunk:
			player_last_chunks[rid] = p_chunk
			trigger_update = true
			
	# Czyszczenie graczy, którzy wyszli z gry
	for rid in player_last_chunks.keys():
		if not rid in current_valid_ids:
			player_last_chunks.erase(rid)
			trigger_update = true
			
	# Jeśli ktoś zmienił chunk, odświeżamy mapę
	if trigger_update:
		update_chunks()
...
func update_chunks():
	...
	# Pobieramy wszystkich aktywnych policjantów
	var players = get_tree().get_nodes_in_group("police")
	if players.is_empty() and player:
		players = [player]
		
	# Zbierz potrzebne chunki dla KAŻDEGO gracza z osobna
	for p in players:
		if not is_instance_valid(p): continue
		var p_x = int(floor(p.global_position.x / chunk_size_px))
		var p_y = int(floor(p.global_position.y / chunk_size_px))
		
		for x in range(p_x - load_radius, p_x + load_radius + 1):
			for y in range(p_y - load_radius, p_y + load_radius + 1):
				var c_id = str(x) + "_" + str(y)
				if not c_id in needed_ids:
					needed_ids.append(c_id)
	
	# Usuwanie starych chunków (tylko jeśli nie są potrzebne ŻADNEMU graczowi)
	...
	# Dodawanie nowych chunków do kolejki ładowania
